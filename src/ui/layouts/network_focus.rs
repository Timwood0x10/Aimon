...
fn create_network_focus_layout(area: Rect) -> Vec<Rect> {
    ...
    vec![main[0], interface_row[0], interface_row[1], interface_row[2], main[2], main[3]]
}
...
pub fn draw(f: &mut Frame, data: &SystemData, history: &HistoryData, theme: &Theme) {
    ...
    for (i, iface) in interfaces.iter().enumerate() {
        ...
        let lines = vec![
            ...
            Line::from(vec![
                ...
                Span::styled(format!("{}", iface.packets_received), Style::default().fg(theme.fg)),
            ]),
            Line::from(vec![
                Span::styled("Pkts TX: ", Style::default().fg(theme.fg)),
                Span::styled(format!("{}", iface.packets_transmitted), Style::default().fg(theme.fg)),
            ]),
        ];
    ...
    for i in interfaces.len()..3 {
        let area = areas[1 + i];
        let block = Paragraph::new("No interface").block(
            Block::default()
                .title(" N/A ")
                .borders(Borders::ALL)
    ...
    let rate_lines = vec![
        Line::from(vec![
            ...
            Span::styled(
                format_rate(rx_rate),
                Style::default().fg(theme.net_rx_color).add_modifier(Modifier::BOLD),
            ),
        ]),
        Line::from(vec![
            ...
            Span::styled(
                format_rate(tx_rate),
                Style::default().fg(theme.net_tx_color).add_modifier(Modifier::BOLD),
            ),
        ]),
    ];

    let rate_block = Paragraph::new(rate_lines).block(
        Block::default()
    ...
    let rx_data: std::collections::VecDeque<u64> = history.network_rx_rate_history.iter().map(|&v| v as u64).collect();
