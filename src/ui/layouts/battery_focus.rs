...
pub fn draw(f: &mut Frame, data: &SystemData, history: &HistoryData, theme: &Theme) {
    ...
    // Battery details
    let status_icon = if batt.is_charging { "CHARGING" } else { "DISCHARGING" };
    ...
    let detail_lines = vec![
        Line::from(vec![
            ...
            Span::styled(status_icon, Style::default().fg(batt_color).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(vec![
            Span::styled("Health: ", Style::default().fg(theme.fg)),
            Span::styled(format!("{:.0}%", batt.health_percentage), Style::default().fg(theme.fg)),
        ]),
        Line::from(vec![
            Span::styled("Cycles: ", Style::default().fg(theme.fg)),
            Span::styled(format!("{}", batt.cycle_count), Style::default().fg(theme.fg)),
        ]),
        Line::from(vec![
        ...
        Line::from(vec![
            ...
            Span::styled(format!("{}W", batt.power_adapter_wattage), Style::default().fg(theme.accent)),
        ]),
    ];
    ...
    let power_lines = vec![
        Line::from(vec![
            ...
            Span::styled(format!("{:.1}W", power.package_w), Style::default().fg(theme.accent).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(vec![
        ...
        Line::from(vec![
            ...
            Span::styled(format!("{:.1}W", power.gpu_w), Style::default().fg(theme.warning_color)),
        ]),
        Line::from(vec![
            Span::styled("ANE:     ", Style::default().fg(theme.fg)),
            Span::styled(format!("{:.1}W", power.ane_w), Style::default().fg(theme.mem_color)),
        ]),
        Line::from(vec![
            Span::styled("Voltage: ", Style::default().fg(theme.fg)),
            Span::styled(format!("{:.2}V", batt.voltage), Style::default().fg(theme.fg)),
        ]),
        Line::from(vec![
            Span::styled("Current: ", Style::default().fg(theme.fg)),
            Span::styled(format!("{:.2}A", batt.amperage), Style::default().fg(theme.fg)),
        ]),
    ];

    let power_block = Paragraph::new(power_lines).block(
        Block::default()
